...
int rand_pipe_y[3] = {0, 0, 0};
int rand_pipe_x[3] = {240, 240 + 96, 240 + 172};
...
int main()
{
    ...
    //map_fragment_info_ptr bg_night;
...
void move_bird(){
    ...
    if (frames == 2) {
        if (bird_y < 110) bird_y = bird_y + 4;// Drop
    }
    update_bird();
    return;
}
void pipesGenerator(){
    //Initial Y position
    int initial_y_pipe_position = random(0, 10);
    // Create the sprite and store the object number
    pipeDownSpt0 = ham_CreateObj((void*)pipe_down_Bitmap, OBJ_SIZE_32X64 , OBJ_MODE_NORMAL,1,0,0,0,0,0,0,rand_pipe_x[0], 0);
    pipeDownSpt1 = ham_CreateObj((void*)pipe_down_Bitmap, OBJ_SIZE_32X64 , OBJ_MODE_NORMAL,1,0,0,0,0,0,0,rand_pipe_x[1], 0);
    pipeDownSpt2 = ham_CreateObj((void*)pipe_down_Bitmap, OBJ_SIZE_32X64 , OBJ_MODE_NORMAL,1,0,0,0,0,0,0,rand_pipe_x[2], 0);
    
    pipeUpSpt0 = ham_CreateObj((void*)pipe_up_Bitmap, OBJ_SIZE_32X64 , OBJ_MODE_NORMAL,1,0,0,0,0,0,0,rand_pipe_x[0], 96 );
    pipeUpSpt1 = ham_CreateObj((void*)pipe_up_Bitmap, OBJ_SIZE_32X64 , OBJ_MODE_NORMAL,1,0,0,0,0,0,0,rand_pipe_x[1], 96 );
    pipeUpSpt2 = ham_CreateObj((void*)pipe_up_Bitmap, OBJ_SIZE_32X64 , OBJ_MODE_NORMAL,1,0,0,0,0,0,0,rand_pipe_x[2], 96 );
    return;
}
void pipesMover(){
    if (frames == 2) {
        ...
        ham_SetObjXY(pipeUpSpt0,rand_pipe_x[0], rand_pipe_y[0] + 96);
            
        ham_SetObjXY(pipeDownSpt1, rand_pipe_x[1], rand_pipe_y[1]);
        ham_SetObjXY(pipeUpSpt1, rand_pipe_x[1], rand_pipe_y[1] + 96 );
        
        ham_SetObjXY(pipeDownSpt2, rand_pipe_x[2], rand_pipe_y[2]);
        ham_SetObjXY(pipeUpSpt2, rand_pipe_x[2],rand_pipe_y[2] + 96 );
